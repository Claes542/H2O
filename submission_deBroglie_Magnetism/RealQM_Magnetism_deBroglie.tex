...
two-valued property posited without a mechanical picture, its $g=2$ arriving through the Dirac equation. The
magnetism of a real atom is then assembled from this operator algebra: orbital and spin angular momenta combine
into $\mathbf J=\mathbf L+\mathbf S$, Hund's rules fix the ground term, and the observable moment carries the
Land\'e $g_J$-factor interpolating between $g_L=1$ and $g_S=2$; open shells are paramagnetic (Curie), closed
shells diamagnetic (Langevin, $\chi\propto-\langle r^2\rangle$), fine structure comes from spin--orbit coupling,
and collective ferromagnetic order from the exchange interaction. This apparatus is quantitatively successful and
is the baseline the present work must reproduce --- but it \emph{posits} spin rather than explaining it, and lives
entirely in operators on a configuration-space amplitude. RealQM takes a different starting point: the fundamental object is not an amplitude on $3N$-dimensional configuration
