...
\title{RealQM and the Electron Gas:\\ a deficiency confined to an idealisation}
...
infinite and the plasmon dispersionless. \textbf{For a real metal the deficiency does not appear.}
...
\section{The metal: where the deficiency does not appear}
...
\begin{itemize}
\item In a \textbf{uniform gas} the mechanism is genuinely absent. $C=0$ is exact, and with it the
screening length is infinite and the plasmon flat. This is a clean statement about the functional.
\item In a \textbf{real metal} the deficiency does not appear at the level a measurement can detect.
...
costs energy when its cells shrink, in the absence of any attractor?} The answer is currently no, and
the honest consequence is that RealQM should not at present be applied to genuinely structureless
electron matter --- degenerate plasma, or the interior of a white dwarf, where the ions are a small
perturbation. Everywhere structure exists, which is everywhere matter is, the question does not
arise.
