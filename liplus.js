...
// Single Li atom (Z=3), 2 electrons in hemispherical shells
...
let sweepResults = [];
let E_min = Infinity;
const hv = 5 / NN, h2v = hv * hv, h3v = hv * hv * hv;
const dv = 0.12;
  ...
  for (let i = 0; i <= NN; i++) {
    ...
    for (let j = 0; j <= NN; j++) {
      ...
      for (let k = 0; k <= NN; k++) {
        ...
        // m=0: hemisphere i < liI (excludes midplane), init exp(-3r)
        if (rLi < R1 && i < liI) {
          Ud[0*S3+id] = Math.exp(-3*rLi); Wd[0*S3+id] = 1;
        }
        // m=1: hemisphere i > liI (excludes midplane), init exp(-3r)
        if (rLi < R1 && i > liI) {
...
function draw() {
  ...
  fill(255);
  text("Li+ 2e | WebGPU, " + NN + "^3, " + (hv * NN).toFixed(0) + " au box", 5, 20);
  ...
  if (phase === 3) {
    fill(200, 100, 100);
    textSize(11);
    text("E_min = " + E_min.toFixed(6) + " Ha  (exact: -7.280 Ha)", 10, 55);
    textSize(9);
  }
}
