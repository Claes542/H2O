...
\title{Unspeakable Quantum Mechanics}
...
declines the creed for the isolated atom --- there the equation's only solution is $\psi=u\,e^{-iEt}$, where
$u$ is the stationary spatial state ($Hu=Eu$) and $e^{-iEt}$ merely a phase factor of modulus one; the density
$|u\,e^{-iEt}|^2=u^2$ is thus constant in time, so this ``evolution'' only spins an unobservable global phase
and there is nothing to run forward. This looks like removing the
