...
tested, placing the threshold field beyond dielectric breakdown. Both point to insulation.
...
\paragraph{Which clock is missing.} ``Static'' is too blunt a word for what is absent, since a wire
...
made nonzero by any refinement of a minimisation. The rate still comes from the dissipation of the
table above.

\paragraph{And a pseudo-time is not a clock.} The solver already advances the interfaces, by the
local density balance $(\rho_i-\rho_j)/(\rho_i+\rho_j)$: first order, monotone in the energy, no
inertia. That is gradient descent, and it can be promoted to a dynamics in exactly two ways, neither
of which answers the question. Overdamped, $\partial_t\Omega=-M\,\delta E/\delta\Omega$, gives a
...
sites}. That removes the mechanism which gives extended systems their large polarisability, and
