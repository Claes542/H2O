...
than conducts. A steady current requires irreversibility, and irreversibility is imported rather
than derived:
...
\noindent Even Landauer's $2e^2/h$ per channel, derived with no collisions anywhere, places its
irreversibility in reservoirs assumed to thermalise; in practice $\sigma=ne^2\tau/m$ is used with
$\tau$ an input.
...
Hamiltonian: two electrons may not share a domain, so $U=\infty$; and the transfer integral
\emph{is} the overlap of neighbouring orbitals, of which there is none, so $t=0$ exactly. In that
