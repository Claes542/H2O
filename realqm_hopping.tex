...
divided by the charge and the hop distance: $E_a/ea=555\,{\rm meV}/6.0\,a_0=1.7\times10^9$~V/m, or
$0.55$~V per lattice site. Copper carrying an ampere runs at $4\times10^{-12}$~V per site. The
framework is therefore eleven orders of magnitude from conducting under any field a material
survives, and the gap is not one a refinement closes.
...
\paragraph{How far $555$~meV can be pushed, and what it therefore cannot decide.} The figure
sits ten per cent above the upper edge of the range real hopping semiconductors occupy, and that
margin is smaller than the systematic uncertainties, which should be set out rather than left
implicit.

The mesh was not held fixed: the spacing $h$ ranged over $0.348$--$0.449$ across the runs that
produced the width curve, varying together with chain length, and the control at matched resolution
was not made. The path was minimised over its \emph{width} only, while a full minimum-energy path
relaxes the kink's shape as well --- an error that is one-sided, since the true saddle can only lie
below the minimum of any restricted family. And the core radius used throughout, $r_c=1.6$, is not
the value calibrated against the isolated atom's ionisation energy, which is $1.93$; no barrier was
computed at the calibrated radius, and $r_c$ demonstrably moves the barrier.

Taken together the figure is $E_a\approx0.5$--$0.6$~eV with a systematic uncertainty of tens of per
cent and a one-sided bias downward. \emph{This article therefore cannot distinguish a barrier just
above the hopping range from one just inside it}, and the reader should not take the comparison
with $500$~meV as settling anything. What the measurement does establish is the order --- half an
electron-volt, not the tens of meV of a good conductor nor the several eV a first fixed-width
calculation suggested --- and that no variation tried reduces it.
...
The honest reading is that the scope claim of \S\ref{sec:scope} is not established, and given the
uncertainties above it is not refuted either. What the framework looks most like is a
\textbf{Mott insulator}: a carrier localised on a site, a gap set by on-site
