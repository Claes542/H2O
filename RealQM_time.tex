...
$\psi=R\,e^{im\varphi}$ --- therefore carries a standing azimuthal current: in cylindrical coordinates
$(s,\varphi,z)$ with $s$ the distance from the axis and $\hat{\boldsymbol\varphi}$ the azimuthal unit vector
(tangent to circles about the axis, in the direction of increasing $\varphi$), one has
$\nabla\varphi=\hat{\boldsymbol\varphi}/s$, so
\[
\mathbf j=\rho\,\nabla(m\varphi)=\frac{m\rho}{s}\,\hat{\boldsymbol\varphi}\neq 0 ,\qquad
\rho\ \text{constant in time},
\]
and hence a magnetic moment $\mu_z=-m\mu_B$. Single-valuedness of $\psi$ forces $m\in\mathbb Z$, so the
circulation is quantised, $\oint\nabla S\cdot d\boldsymbol\ell=2\pi m$: the state is a quantised charge
\emph{vortex} carrying orbital angular momentum $L_z=m\hbar$, its density vanishing on the axis (where
$\mathbf v=m/s$ would otherwise diverge) --- a nodal line, hollow along the axis. The density does not change;
the flow is steady. This current is
...
\paragraph{A real route --- magnetism needs a velocity, not the complex plane.} It is worth separating what
here is genuinely ``complex'' from what is mere bookkeeping. From the density \emph{alone} --- a single real
field $\psi=R$ --- no magnetism can come, since $\mathbf j=\operatorname{Im}(\psi^*\nabla\psi)\equiv0$: a lone
amplitude carries no motion. But complex numbers are not the only way to carry a second field. Continuum
mechanics never uses them, yet always carries \emph{two} real fields, density $\rho$ and velocity $\mathbf v$;
do the same here and the whole of magnetism is real. The current is $\mathbf j=\rho\,\mathbf v$, a real
circulating flow ($\nabla\!\cdot\mathbf j=0$); the flow costs a real kinetic energy
\[
T_{\text{flow}}=\int \tfrac12\rho\,\mathbf v^2=\int \frac{\mathbf j^2}{2\rho} ,
\]
it carries the real angular momentum $L_z=\int\rho\,(\mathbf x\times\mathbf v)_z\,dV$ and the ordinary
magnetostatic moment $\boldsymbol\mu=\tfrac12\int \mathbf x\times\mathbf j\,dV$, and the magnetic state is
obtained by minimising the static energy \emph{augmented by} $T_{\text{flow}}$ at fixed $L_z$ (or, in a field,
minimising $E-\boldsymbol\mu\!\cdot\!\mathbf B$). No $i$ enters any of this: it is a real charge vortex, exactly
the $(\rho,\mathbf v)$ of continuum mechanics an \aa ngstr\"om across. The complex $\psi=\sqrt\rho\,e^{iS}$ is
simply these two real fields repackaged into one symbol, with $\mathbf v=\nabla S$; the phase \emph{is} the
velocity potential. The one thing the real $(\rho,\mathbf v)$ theory cannot supply on its own is the
\emph{integer}: single-valuedness of $e^{iS}$ (the phase is an \emph{angle}, defined mod $2\pi$) forces the
quantised circulation $\oint\mathbf v\cdot d\boldsymbol\ell=2\pi m$, $m\in\mathbb Z$, which a purely real
theory would have to impose as a separate condition (the Onsager--Feynman vortex quantisation of real
superfluids). What is unavoidable there is the \emph{circle}, not the complex plane: magnetism's dynamics,
energy and moment are real; only the quantum integer $m$ needs the periodic phase.
