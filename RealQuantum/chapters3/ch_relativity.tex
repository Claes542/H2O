...
less energy in the denominator. So $E=mc^2$, here, is not a conversion RealQM borrows but a statement it should
\emph{derive} --- that electromagnetic self-inertia equals energy. Two things stay open and are named as such: the
step $\mathbf p=(E/c^2)\mathbf v$ must come from RealQM's own charge dynamics (the electromagnetic-mass problem,
with its factor $4/3$ that only the full stress--energy fixes to $E/c^2$), and absolute rest energies, not only
binding differences, must eventually be computed. The elimination of mass is thus demonstrated for the
\emph{differences} a Penning trap actually measures, and set as a target for the rest. A fuller treatment is
given in the companion note \emph{Energy Without Mass: RealQM, Inertia, and the Penning Trap}.
